...
const Lesson1Model = ({ isLightOn }) => {
    ...
    const loadRobotModel = (scene) => {
        const loader = new GLTFLoader();
        loader.load('/robo1/scene.gltf', (gltf) => {
            robotRef.current = gltf.scene;
            robotRef.current.scale.set(1.5, 1.5, 1.5);
    ...
    useEffect(() => {
        if (directionalLightRef.current) {
            directionalLightRef.current.intensity = isLightOn ? 1 : 0;
        }
    }, [isLightOn]);
